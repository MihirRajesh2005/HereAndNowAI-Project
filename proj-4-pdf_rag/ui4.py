...
with gr.Blocks(theme="default", title=branding["organizationName"]) as demo:
    ...
    gr.ChatInterface(fn=get_response,
                     chatbot=gr.Chatbot(height=500, avatar_images=(None,branding["chatbot"] ["avatar"]), type="messages"),
                     title=branding["organizationName"],
                     description=branding["slogan"],
                     type="messages",
                     examples=[["What is Here and Now AI"], ["Who are the founders"], ["what does the company do"], ["when was it founded"]])
